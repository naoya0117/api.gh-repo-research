...
func main() {
	var (
		start     = flag.Bool("start", false, "Start new batch analysis")
		resume    = flag.Bool("resume", false, "Resume batch analysis")
	...
	if !*start && !*resume && !*status {
		fmt.Println("Usage:")
		fmt.Println("  --start                 Start new batch analysis")
		fmt.Println("  --resume --session-id=X Resume batch analysis")
		fmt.Println("  --status                Show current status")
		fmt.Println("")
		fmt.Println("Options:")
		fmt.Println("  --work-dir=DIR          Working directory (default: tmp_repositories)")
		fmt.Println("  --db-url=URL            Database URL")
		os.Exit(1)
	}
...
func setupDatabase(dbURL string) (*database.DB, error) {
	if dbURL == "" {
...
func printStatus(status *batch.AnalysisStatus) {
